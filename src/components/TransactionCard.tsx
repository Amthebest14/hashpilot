...
export default function TransactionCard({
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      // Support both legacy string return and new TreasuryResult return
      const explorerUrl = result && typeof result === 'object' ? result.explorerUrl : result;
      setHash(explorerUrl);
      setStatus('success');
      ...
      if (address) {
        awardHP(address, 15, hederaId).then(() => {
          toast.success('⚡ +15 HP REWARDED FOR TREASURY TRANSFER');
  ...
  // Calculate live conversion values using oracle prices
  ...
  // Define dynamic button text and descriptions
  let actionLabel = 'Execute Treasury Transaction';
  if (intent === 'pay_service') {
    actionLabel = `Pay Vendor in ${tokenSymbol}`;
  } else if (intent === 'transfer_token') {
    actionLabel = `Send ${tokenSymbol} Now`;
  }
    ...
    <div className={`w-full max-w-md my-4 animate-in fade-in slide-in-from-bottom-4 duration-500 ${isEffectivelyExpired ? 'opacity-50' : ''}`}>
      <div className="bg-gradient-to-b from-[#161a26] to-[#0f111a] border border-[#23293d] rounded-3xl overflow-hidden shadow-2xl relative">
        ...
        <div className="p-6 space-y-5">
          ...
          {status === 'error' && errorMsg && (
            <div className="p-3 bg-red-500/10 border border-red-500/25 rounded-2xl">
               <span className="text-[10px] font-mono text-red-400 leading-tight block break-words">
                 TRANSACTION_REJECTED: {errorMsg.substring(0, 150)}{errorMsg.length > 150 ? '...' : ''}
               </span>
            </div>
          )}
          ...
          {/* Large Payment Value Presentation */}
